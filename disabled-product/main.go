...
func handler(request events.APIGatewayProxyRequest) (events.APIGatewayProxyResponse, error) {
	...
	if bodyData.Active > 1 {
		var response Response
		response.Success = false
		response.Message = "El parámetro Active no puede ser mayor a 1"
		return returnApiGateway(response, 401)
	}
	...
	update, err := db.Exec("UPDATE products SET active = ? WHERE id = ? AND id_user = ?", bodyData.Active, bodyData.IDProduct, bodyData.IDUser)
	if err != nil {
		panic(err.Error())
	}
	...
	if row <= 0 {
		var res Response
		res.Success = false
		res.Message = "No se pudieron actualizar los datos."
