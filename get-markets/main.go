...
func handler(request events.APIGatewayProxyRequest) (events.APIGatewayProxyResponse, error) {
	...
	go func() {
		defer wg.Done()
		results, err := db.Query("SELECT id, id_postal_code, type_market, name FROM cat_markets where id_postal_code = ? and active = 1 ORDER BY id ASC", request.PathParameters["id_cp"])
		if err != nil {
			panic(err)
		}
