...
func handler(request events.APIGatewayProxyRequest) (events.APIGatewayProxyResponse, error) {
	...
	update, err := db.Exec("UPDATE products SET name = ?, price_pz = ?, price_kg = ? WHERE id = ? AND id_user = ?", bodyData.Name, bodyData.PricePZ, bodyData.PriceKG, bodyData.IDProduct, bodyData.IDUser)
	if err != nil {
		panic(err.Error())
	}
